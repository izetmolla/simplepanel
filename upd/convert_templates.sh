...
sed -i "s/apache/httpd/g" /usr/local/sfpanel/conf/sfpanel.conf

# Rename dns system service
sed -i "s/bind/named/g" /usr/local/sfpanel/conf/sfpanel.conf

# Rename nginx config
mv /etc/nginx/conf.d/sfpanel_users.conf /etc/nginx/conf.d/sfpanel.conf 2>/dev/null
rm -f /etc/nginx/conf.d/sfpanel_ip.conf 2>/dev/null

# Update user packages
PKG=/usr/local/sfpanel/data/packages
...
# Update users
USR=/usr/local/sfpanel/data/users
...
sed -i "s/NGINX/PROXY/g" /usr/local/sfpanel/data/users/*/web.conf

# Check template structure
TPL='/usr/local/sfpanel/data/templates/web'
...
# Update version
sed -i 's/0.9.7/0.9.8/' /usr/local/sfpanel/conf/sfpanel.conf
