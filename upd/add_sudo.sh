...
if [ ! -e '/etc/sudoers.d/admin' ]; then
    ...
    echo '# Created by sfpanel update-trigger' > /etc/sudoers.d/admin
    echo 'Defaults env_keep="SFPANEL"' >> /etc/sudoers.d/admin
    ...
    echo 'admin   ALL=NOPASSWD:/usr/local/sfpanel/bin/*' >> /etc/sudoers.d/admin
