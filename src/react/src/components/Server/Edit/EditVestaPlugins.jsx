...
const EditSfpanelPluginsOption = ({ data, visible }) => {
    ...
    if (sftpValue === 'yes') {
      if (!data.sftp_license_key && session['SFTPJAIL_KEY']) {
        ...
        return (<div className="sftp-module">
          ...
          <div className="buy-license">
            <a href={`https://sfpanel.izetmolla.com/checkout/2co.php?product_id=6&referer=${data.http_host}`}>
              {i18n['Buy Licence']} 3$ {i18n['month']}
            </a>
            <a href={`https://sfpanel.izetmolla.com/checkout/2co.php?product_id=9&referer=${data.http_host}`}>
              {i18n['Buy Lifetime License']} 18$
            </a>
          </div>

          <span>2Checkout.com Inc. (Ohio, USA) is a payment facilitator for goods and services provided by sfpanel.izetmolla.com.</span>
        </div>)
      }
    }
  }
    ...
    if (fmValue === 'yes') {
      if (!data.fm_license_key && session['FILEMANAGER_KEY']) {
        ...
        return (<div className="fm-module">
          ...
          <div className="buy-license">
            <a href={`https://sfpanel.izetmolla.com/checkout/2co.php?product_id=7&referer=${data.http_host}`} target="_blank" rel="noopener noreferrer">
              {i18n['Buy Licence']} 3$ {i18n['month']}
            </a>
            <a href={`https://sfpanel.izetmolla.com/checkout/2co.php?product_id=8&referer=${data.http_host}`} target="_blank" rel="noopener noreferrer">
...
export default EditSfpanelPluginsOption;
