...
if (isset($_SESSION['user'])) {
    ...
    if ($_SESSION['user'] == 'admin' && !empty($_GET['loginas'])) {
        exec (SFPANEL_CMD . "v-list-user ".escapeshellarg($_GET['loginas'])." json", $output, $return_var);
        if ( $return_var == 0 ) {
    	...
    	exec (SFPANEL_CMD . "v-list-user ".$v_user." json", $output, $return_var);
        $users = json_decode(implode('', $output), true);
    }
}
...
if (isset($_POST['user']) && isset($_POST['password'])) {
    if(isset($_SESSION['token']) && isset($_POST['token']) && $_POST['token'] == $_SESSION['token']) {
        ...
        if($_POST['user'] == 'root'){
            ...
            // Get user's salt
            $output = '';
            exec (SFPANEL_CMD."v-get-user-salt ".$v_user." ".$v_ip." json" , $output, $return_var);
            ...
            if ( $return_var > 0 ) {
                ...
                // Check user hash
                exec(SFPANEL_CMD ."v-check-user-hash ".$v_user." ".$v_hash." ".$v_ip,  $output, $return_var);
                unset($output);
