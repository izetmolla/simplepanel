...
if (!empty($_GET['type'])) {
    $restore_cmd = SFPANEL_CMD."v-schedule-user-restore ".$user." ".$backup." ".$web." ".$dns." ".$mail." ".$db." ".$cron." ".$udir;
} else {
    $restore_cmd = SFPANEL_CMD."v-schedule-user-restore ".$user." ".$backup;
}
...
if ($return_var == 0) {
    ...
    if (empty($_SESSION['error_msg'])) {
        $_SESSION['error_msg'] = __('Error: sfpanel did not return any output.');
    }
    if ($return_var == 4) {
