...
if (!empty($_POST['ok'])) {
    ...
    if (empty($_SESSION['error_msg'])) {
        exec (SFPANEL_CMD."v-add-mail-domain ".$user." ".$v_domain." ".$v_antispam." ".$v_antivirus." ".$v_dkim, $output, $return_var);
...
if (!empty($_POST['ok_acc'])) {
    ...
    if (empty($_SESSION['error_msg'])) {
        ...
        fclose($fp);
        exec (SFPANEL_CMD."v-add-mail-account ".$user." ".$v_domain." ".$v_account." ".$v_password." ".$v_quota, $output, $return_var);
        check_return_code($return_var,$output);
        unset($output);
        unlink($v_password);
    ...
    if ((!empty($_POST['v_aliases'])) && (empty($_SESSION['error_msg']))) {
        ...
        foreach ($aliases as $alias) {
            ...
            if (empty($_SESSION['error_msg'])) {
                exec (SFPANEL_CMD."v-add-mail-account-alias ".$user." ".$v_domain." ".$v_account." ".$alias, $output, $return_var);
                check_return_code($return_var,$output);
                unset($output);
            }
        }
    ...
    if ((!empty($_POST['v_fwd'])) && (empty($_SESSION['error_msg']))) {
        ...
        foreach ($fwd as $forward) {
            ...
            if (empty($_SESSION['error_msg'])) {
                exec (SFPANEL_CMD."v-add-mail-account-forward ".$user." ".$v_domain." ".$v_account." ".$forward, $output, $return_var);
                check_return_code($return_var,$output);
                unset($output);
            }
        }
    ...
    if ((!empty($_POST['v_fwd_only'])) && (empty($_SESSION['error_msg']))) {
        exec (SFPANEL_CMD."v-add-mail-account-fwd-only ".$user." ".$v_domain." ".$v_account, $output, $return_var);
        check_return_code($return_var,$output);
        unset($output);
    }
