...
if (empty($_GET['backup'])){
  exec (SFPANEL_CMD."v-list-user-backups $user json", $output, $return_var);
  ...
  exec (SFPANEL_CMD."v-list-user-backup $user ".escapeshellarg($_GET['backup'])." json", $output, $return_var);
  $data = json_decode(implode('', $output), true);
  $data = array_reverse($data,true);
  unset($output);
