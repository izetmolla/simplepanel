<?php
define('SFPANEL_CMD', '/usr/bin/sudo /usr/local/sfpanel/bin/');
...
if (isset($_POST['user']) || isset($_POST['hash'])) {
    ...
    if (empty($_POST['hash'])) {
        ...
        $output = '';
        exec (SFPANEL_CMD."v-get-user-salt admin ".$v_ip." json" , $output, $return_var);
        ...
        // Check user hash
        exec(SFPANEL_CMD ."v-check-user-hash admin ".$v_hash." ".$v_ip,  $output, $return_var);
        unset($output);
        ...
        $key = '/usr/local/sfpanel/data/keys/' . basename($_POST['hash']);
        if (file_exists($key) && is_file($key)) {
            exec(SFPANEL_CMD ."v-check-api-key ".escapeshellarg($key)." ".$v_ip,  $output, $return_var);
            unset($output);
