...
is_web_domain_new() {
    web=$(grep -F -H "DOMAIN='$1'" $SFPANEL/data/users/*/web.conf)
    if [ ! -z "$web" ]; then
        if [ "$type" == 'web' ]; then
...
is_web_alias_new() {
    web_alias=$(grep -wH "$1" $SFPANEL/data/users/*/web.conf)
    if [ ! -z "$web_alias" ]; then
...
prepare_web_domain_values() {
    ...
    if [ "$SUSPENDED" = 'yes' ]; then
        docroot="$SFPANEL/data/templates/web/suspend"
        sdocroot="$SFPANEL/data/templates/web/suspend"
    fi
}
...
add_web_config() {
    ...
    if [ -z "$(grep "$conf" /etc/$1/conf.d/sfpanel.conf)" ]; then
        if [ "$1" != 'nginx' ]; then
            echo "Include $conf" >> /etc/$1/conf.d/sfpanel.conf
        else
            echo "include $conf;" >> /etc/$1/conf.d/sfpanel.conf
        fi
    fi
...
del_web_config() {
    ...
    if [ -e "$conf" ]; then
        sed -i "\|$conf|d" /etc/$1/conf.d/sfpanel.conf
        rm -f $conf
    else
        # fallback to old style configs
        conf="$HOMEDIR/$user/conf/web/$1.conf"
    ...
    if [ "$web_domain" -eq '0' ]; then
        sed -i "/.*\/$user\/conf\/web\//d" /etc/$1/conf.d/sfpanel.conf
        if [ -f "$conf" ]; then
            rm -f $conf
        fi
    fi
...
is_dns_domain_new() {
    dns=$(ls $SFPANEL/data/users/*/dns/$1.conf 2>/dev/null)
    if [ ! -z "$dns" ]; then
        if [ "$2" == 'dns' ]; then
...
is_mail_domain_new() {
    mail=$(ls $SFPANEL/data/users/*/mail/$1.conf 2>/dev/null)
    if [ ! -z "$mail" ]; then
        if [ "$2" == 'mail' ]; then
