...
send_ssh_cmd() {
    ...
    if [ "$USER" = 'root' ]; then
        args="$SFPANEL/bin/$1 \"$2\" \"$3\" \"$4\" \"$5\""
    else
        args="sudo $SFPANEL/bin/$1 \"$2\" \"$3\" \"$4\" \"$5\""
    fi
...
is_dnshost_new() {
    if [ -e "$SFPANEL/conf/dns-cluster.conf" ]; then
        check_host=$(grep "HOST='$host'" $SFPANEL/conf/dns-cluster.conf)
...
remote_dns_health_check() {
    ...
    for str in $(grep "SUSPENDED='no'" $SFPANEL/conf/dns-cluster.conf); do
        eval $str
