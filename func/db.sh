# MySQL
mysql_connect() {
    host_str=$(grep "HOST='$1'" $SFPANEL/conf/mysql.conf)
    eval $host_str
...
# PostgreSQL
psql_connect() {
    host_str=$(grep "HOST='$1'" $SFPANEL/conf/pgsql.conf)
    eval $host_str
...
is_charset_valid() {
    host_str=$(grep "HOST='$host'" $SFPANEL/conf/$type.conf)
    eval $host_str
...
increase_dbhost_values() {
    ...
    sed -i "s/$old_dbbases/$new_dbbases/g" $SFPANEL/conf/$type.conf
    sed -i "s/$old_users/$new_users/g" $SFPANEL/conf/$type.conf
}
...
is_dbhost_new() {
    if [ -e "$SFPANEL/conf/$type.conf" ]; then
        check_host=$(grep "HOST='$host'" $SFPANEL/conf/$type.conf)
