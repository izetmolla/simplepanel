...
RRD_STEP=300
BIN=$SFPANEL/bin
USER_DATA=$SFPANEL/data/users/$user
WEBTPL=$SFPANEL/data/templates/web
DNSTPL=$SFPANEL/data/templates/dns
RRD=$SFPANEL/web/rrd
SENDMAIL="$SFPANEL/web/inc/mail-wrapper.php"
...
log_event() {
    ...
    if [ "$1" -eq 0 ]; then
        echo "$LOG_TIME $2" >> $SFPANEL/log/system.log
    else
        echo "$LOG_TIME $2 [Error $1]" >> $SFPANEL/log/error.log
    fi
}
...
log_history() {
    ...
    undo=${2-no}
    log_user=${3-$user}
    log=$SFPANEL/data/users/$log_user/history.log
    touch $log
...
# Check user backup settings
is_backup_scheduled() {
    if [ -e "$SFPANEL/data/queue/backup.pipe" ]; then
        check_q=$(grep " $user " $SFPANEL/data/queue/backup.pipe | grep $1)
        if [ ! -z "$check_q" ]; then
...
is_object_valid() {
    if [ $2 = 'USER' ]; then
        ...
        if [ ! -d "$SFPANEL/data/users/$user_vst_dir" ]; then
            check_result $E_NOTEXIST "$1 $3 doesn't exist"
        fi
    else
        object=$(grep "$2='$3'" $SFPANEL/data/users/$user/$1.conf)
        if [ -z "$object" ]; then
            arg1=$(basename $1)
...
update_user_value() {
    key="${2//$}"
    lnr=$(grep -n "^$key='" $SFPANEL/data/users/$1/user.conf |cut -f 1 -d ':')
    if [ ! -z "$lnr" ]; then
        sed -i "$lnr d" $SFPANEL/data/users/$1/user.conf
        sed -i "$lnr i\\$key='${3}'" $SFPANEL/data/users/$1/user.conf
    fi
}
...
increase_user_value() {
    key="${2//$}"
    factor="${3-1}"
    conf="$SFPANEL/data/users/$1/user.conf"
...
# $2 = body
send_email_to_admin() {
    email=$(grep CONTACT /usr/local/sfpanel/data/users/admin/user.conf)
