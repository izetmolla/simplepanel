# Check ip ownership
is_ip_owner() {
    owner=$(grep 'OWNER=' $SFPANEL/data/ips/$ip |cut -f 2 -d \')
    if [ "$owner" != "$user" ]; then
...
is_ip_key_empty() {
    key="$1"
    string=$(cat $SFPANEL/data/ips/$ip)
    eval $string
    eval value="$key"
...
# Get ip name
get_ip_alias() {
    ip_name=$(grep "NAME=" $SFPANEL/data/ips/$local_ip |cut -f 2 -d \')
    if [ ! -z "$ip_name" ]; then
        echo "${1//./-}.$ip_name"
    fi
}
...
increase_ip_value() {
    ...
    usr_key='U_SYS_USERS'
    current_web=$(grep "$web_key=" $SFPANEL/data/ips/$sip |cut -f 2 -d \')
    current_usr=$(grep "$usr_key=" $SFPANEL/data/ips/$sip |cut -f 2 -d \')
...
# Get real ip address
get_real_ip() {
    if [ -e "$SFPANEL/data/ips/$1" ]; then
        echo $1
    else
        nat=$(grep -H "^NAT='$1'" $SFPANEL/data/ips/*)
        if [ ! -z "$nat" ]; then
...
# Get user ips
get_user_ips() {
    dedicated=$(grep -H "OWNER='$user'" $SFPANEL/data/ips/*)
    dedicated=$(echo "$dedicated" |cut -f 1 -d : |sed 's=.*/==')
    shared=$(grep -H -A1 "OWNER='admin'" $SFPANEL/data/ips/* |grep shared)
...
get_user_ip() {
    ...
    local_ip=$ip
    nat=$(grep "^NAT" $SFPANEL/data/ips/$ip |cut -f 2 -d \')
    if [ ! -z "$nat" ]; then
        ip=$nat
    fi
}
...
is_ip_valid() {
    ...
    if [ ! -z $2 ]; then
        if [ -z "$nat" ]; then
            ip_data=$(cat $SFPANEL/data/ips/$1)
        else
            ip_data=$(cat $SFPANEL/data/ips/$nat)
        fi
