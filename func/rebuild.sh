...
rebuild_user_conf() {
    ...
    if [ -x "$SFPANEL/data/packages/$PACKAGE.sh" ]; then
        $SFPANEL/data/packages/$PACKAGE.sh "$user" "$CONTACT" "$FNAME" "$LNAME"
    fi
    ...
    # Update disk pipe
    sed -i "/ $user$/d" $SFPANEL/data/queue/disk.pipe
    echo "$BIN/v-update-user-disk $user" >> $SFPANEL/data/queue/disk.pipe
    ...
    if [ ! -z "$WEB_SYSTEM" ] && [ "$WEB_SYSTEM" != 'no' ]; then
        ...
        if [ "$(grep -w $user $SFPANEL/data/queue/traffic.pipe)" ]; then
            echo "$BIN/v-update-web-domains-traff $user" \
                >> $SFPANEL/data/queue/traffic.pipe
        fi
        echo "$BIN/v-update-web-domains-disk $user" \
            >> $SFPANEL/data/queue/disk.pipe
    ...
    if [ ! -z "$DB_SYSTEM" ] && [ "$DB_SYSTEM" != 'no' ]; then
        ...
        echo "$BIN/v-update-databases-disk $user" >> $SFPANEL/data/queue/disk.pipe
...
rebuild_web_domain_conf() {
    ...
    if [ ! -z "$STATS" ]; then
        ...
        check_webstats=$(grep "$webstats" $SFPANEL/data/queue/webstats.pipe)
        if [ -z "$check_webstats" ]; then
            echo "$webstats" >> $SFPANEL/data/queue/webstats.pipe
        fi
...
rebuild_pgsql_database() {
    ...
    host_str=$(grep "HOST='$HOST'" $SFPANEL/conf/pgsql.conf)
    eval $host_str
...
import_mysql_database() {
    ...
    host_str=$(grep "HOST='$HOST'" $SFPANEL/conf/mysql.conf)
    eval $host_str
