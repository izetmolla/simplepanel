...
RHOST='apt.sfpanel.izetmolla.com'
...
sfpanelcp="$SFPANEL/install/$VERSION/$release"
...
software="nginx apache2 apache2.2-common apache2-suexec-custom apache2-utils
    ...
    sudo vim-common vsftpd webalizer whois zip net-tools"
...
help() {
  ...
  -d, --port              Set Sfpanel port
  -p, --password          Set admin password
  -f, --force             Force installation
  -h, --help              Print this help

  Example: bash $0 -e demo@sfpanel.izetmolla.com -p p4ssw0rd --apache no --phpfpm yes"
    exit 1
}
...
# wget -q "c.sfpanel.izetmolla.com/deb_signing.key" -O /dev/null
# check_result $? "No access to Sfpanel repository"
...
# fi
# Checking conflicts
for pkg in exim4 mysql-server apache2 nginx sfpanel; do
    if [ ! -z "$(grep $pkg $tmpfile)" ]; then
...
clear
echo
echo '  _____  ______  _____          _   _  ______  _'   
echo ' / ____||  ____||  __ \  /\    | \ | ||  ____|| |'
echo '| (___  | |__   | |__) |/  \   |  \| || |__   | |'
echo ' \___ \ |  __|  |  ___// /\ \  | . ` ||  __|  | |'
echo ' ____) || |     | |   / ____ \ | |\  || |____ | |____'
echo '|_____/ |_|     |_|  /_/    \_\|_| \_||______||______|'                                                 
echo
echo '                                  Sfpanel Control Panel'
echo -e "\n\n"
...
if [ "$interactive" = 'yes' ]; then
    ...
    if [ -z "$email" ]; then
     ...
     # Asking for Sfpanel port
    if [ -z "$port" ]; then
        read -p 'Please enter Sfpanel port number (press enter for 8083): ' port
    fi
...
# # Installing sfpanel repo
# echo "deb http://$RHOST/$codename/ $codename sfpanel" > $apt/sfpanel.list
...
mkdir spamassassin mysql postgresql mongodb sfpanel
...
# Backup Sfpanel
service sfpanel stop > /dev/null 2>&1
cp -r $SFPANEL/* $vst_backups/sfpanel > /dev/null 2>&1
# apt-get -y remove sfpanel sfpanel-nginx sfpanel-php > /dev/null 2>&1
# apt-get -y purge sfpanel sfpanel-nginx sfpanel-php > /dev/null 2>&1
# rm -rf $SFPANEL > /dev/null 2>&1
...
cp -f $sfpanelcp/sudo/admin /etc/sudoers.d/
...
chmod 755 /etc/profile.d/sfpanel.sh
source /etc/profile.d/sfpanel.sh
echo 'PATH=$PATH:'$SFPANEL'/bin' >> /root/.bash_profile
echo 'export PATH' >> /root/.bash_profile
source /root/.bash_profile

# Configuring logrotate for Sfpanel logs
cp -f $sfpanelcp/logrotate/sfpanel /etc/logrotate.d/

# Building directory tree and creating some blank files for Sfpanel
mkdir -p $SFPANEL/conf $SFPANEL/log $SFPANEL/ssl $SFPANEL/data/ips \
    $SFPANEL/data/queue $SFPANEL/data/users $SFPANEL/data/firewall \
    $SFPANEL/data/sessions
touch $SFPANEL/data/queue/backup.pipe $SFPANEL/data/queue/disk.pipe \
    $SFPANEL/data/queue/webstats.pipe $SFPANEL/data/queue/restart.pipe \
    $SFPANEL/data/queue/traffic.pipe $SFPANEL/log/system.log \
    $SFPANEL/log/nginx-error.log $SFPANEL/log/auth.log
chmod 750 $SFPANEL/conf $SFPANEL/data/users $SFPANEL/data/ips $SFPANEL/log
chmod -R 750 $SFPANEL/data/queue
chmod 660 $SFPANEL/log/*
rm -f /var/log/sfpanel
ln -s $SFPANEL/log /var/log/sfpanel
chmod 770 $SFPANEL/data/sessions

# Generating Sfpanel configuration
rm -f $SFPANEL/conf/sfpanel.conf 2>/dev/null
touch $SFPANEL/conf/sfpanel.conf
chmod 660 $SFPANEL/conf/sfpanel.conf

# Web stack
if [ "$apache" = 'yes' ] && [ "$nginx" = 'no' ] ; then
    echo "WEB_SYSTEM='apache2'" >> $SFPANEL/conf/sfpanel.conf
    echo "WEB_RGROUPS='www-data'" >> $SFPANEL/conf/sfpanel.conf
    echo "WEB_PORT='80'" >> $SFPANEL/conf/sfpanel.conf
    echo "WEB_SSL_PORT='443'" >> $SFPANEL/conf/sfpanel.conf
    echo "WEB_SSL='mod_ssl'"  >> $SFPANEL/conf/sfpanel.conf
    echo "STATS_SYSTEM='webalizer,awstats'" >> $SFPANEL/conf/sfpanel.conf
fi
if [ "$apache" = 'yes' ] && [ "$nginx"  = 'yes' ] ; then
    echo "WEB_SYSTEM='apache2'" >> $SFPANEL/conf/sfpanel.conf
    echo "WEB_RGROUPS='www-data'" >> $SFPANEL/conf/sfpanel.conf
    echo "WEB_PORT='8080'" >> $SFPANEL/conf/sfpanel.conf
    echo "WEB_SSL_PORT='8443'" >> $SFPANEL/conf/sfpanel.conf
    echo "WEB_SSL='mod_ssl'"  >> $SFPANEL/conf/sfpanel.conf
    echo "PROXY_SYSTEM='nginx'" >> $SFPANEL/conf/sfpanel.conf
    echo "PROXY_PORT='80'" >> $SFPANEL/conf/sfpanel.conf
    echo "PROXY_SSL_PORT='443'" >> $SFPANEL/conf/sfpanel.conf
    echo "STATS_SYSTEM='webalizer,awstats'" >> $SFPANEL/conf/sfpanel.conf
fi
if [ "$apache" = 'no' ] && [ "$nginx"  = 'yes' ]; then
    echo "WEB_SYSTEM='nginx'" >> $SFPANEL/conf/sfpanel.conf
    echo "WEB_PORT='80'" >> $SFPANEL/conf/sfpanel.conf
    echo "WEB_SSL_PORT='443'" >> $SFPANEL/conf/sfpanel.conf
    echo "WEB_SSL='openssl'"  >> $SFPANEL/conf/sfpanel.conf
    if [ "$phpfpm" = 'yes' ]; then
        echo "WEB_BACKEND='php-fpm'" >> $SFPANEL/conf/sfpanel.conf
    fi
    echo "STATS_SYSTEM='webalizer,awstats'" >> $SFPANEL/conf/sfpanel.conf
fi
...
# FTP stack
if [ "$vsftpd" = 'yes' ]; then
    echo "FTP_SYSTEM='vsftpd'" >> $SFPANEL/conf/sfpanel.conf
fi
if [ "$proftpd" = 'yes' ]; then
    echo "FTP_SYSTEM='proftpd'" >> $SFPANEL/conf/sfpanel.conf
fi

# DNS stack
if [ "$named" = 'yes' ]; then
    echo "DNS_SYSTEM='bind9'" >> $SFPANEL/conf/sfpanel.conf
fi

# Mail stack
if [ "$exim" = 'yes' ]; then
    echo "MAIL_SYSTEM='exim4'" >> $SFPANEL/conf/sfpanel.conf
    if [ "$clamd" = 'yes'  ]; then
        echo "ANTIVIRUS_SYSTEM='clamav-daemon'" >> $SFPANEL/conf/sfpanel.conf
    fi
    if [ "$spamd" = 'yes' ]; then
        echo "ANTISPAM_SYSTEM='spamassassin'" >> $SFPANEL/conf/sfpanel.conf
    fi
    if [ "$dovecot" = 'yes' ]; then
        echo "IMAP_SYSTEM='dovecot'" >> $SFPANEL/conf/sfpanel.conf
    fi
fi

# Cron daemon
echo "CRON_SYSTEM='cron'" >> $SFPANEL/conf/sfpanel.conf
...
if [ "$iptables" = 'yes' ] && [ "$fail2ban" = 'yes' ]; then
    echo "FIREWALL_EXTENSION='fail2ban'" >> $SFPANEL/conf/sfpanel.conf
fi

# Disk quota
if [ "$quota" = 'yes' ]; then
    echo "DISK_QUOTA='yes'" >> $SFPANEL/conf/sfpanel.conf
fi

# Backups
echo "BACKUP_SYSTEM='local'" >> $SFPANEL/conf/sfpanel.conf

# Language
echo "LANGUAGE='$lang'" >> $SFPANEL/conf/sfpanel.conf

# Version
echo "VERSION='0.9.8'" >> $SFPANEL/conf/sfpanel.conf

# Installing hosting packages
cp -rf $sfpanelcp/packages $SFPANEL/data/

# Installing templates
cp -rf $sfpanelcp/templates $SFPANEL/data/

# Copying index.html to default documentroot
cp $SFPANEL/data/templates/web/skel/public_html/index.html /var/www/
sed -i 's/%domain%/It worked!/g' /var/www/index.html

# Installing firewall rules
cp -rf $sfpanelcp/firewall $SFPANEL/data/

# Configuring server hostname
$SFPANEL/bin/v-change-sys-hostname $servername 2>/dev/null

# Generating SSL certificate
$SFPANEL/bin/v-generate-ssl-cert $(hostname) $email 'US' 'California' \
     'San Francisco' 'Sfpanel Control Panel' 'IT' > /tmp/vst.pem
...
chown root:mail $SFPANEL/ssl/*
chmod 660 $SFPANEL/ssl/*
rm /tmp/vst.pem
...
if [ "$nginx" = 'yes' ]; then
    ...
    cp -f $sfpanelcp/nginx/nginx.conf /etc/nginx/
    cp -f $sfpanelcp/nginx/status.conf /etc/nginx/conf.d/
    cp -f $sfpanelcp/nginx/phpmyadmin.inc /etc/nginx/conf.d/
    cp -f $sfpanelcp/nginx/phppgadmin.inc /etc/nginx/conf.d/
    cp -f $sfpanelcp/nginx/webmail.inc /etc/nginx/conf.d/
    cp -f $sfpanelcp/logrotate/nginx /etc/logrotate.d/
    echo > /etc/nginx/conf.d/sfpanel.conf
...
if [ "$apache" = 'yes'  ]; then
    cp -f $sfpanelcp/apache2/apache2.conf /etc/apache2/
    cp -f $sfpanelcp/apache2/status.conf /etc/apache2/mods-enabled/
    cp -f  $sfpanelcp/logrotate/apache2 /etc/logrotate.d/
    a2enmod rewrite
    a2enmod suexec
    a2enmod ssl
    a2enmod actions
    ...
    echo > /etc/apache2/conf.d/sfpanel.conf
    echo "# Server control panel by SFPANEL" > /etc/apache2/sites-available/default
    echo "# Server control panel by SFPANEL" > /etc/apache2/sites-available/default-ssl
    echo "# Server control panel by SFPANEL" > /etc/apache2/ports.conf
...
if [ "$phpfpm" = 'yes' ]; then
    ...
    cp -f $sfpanelcp/php-fpm/www.conf $pool/
...
if [ "$vsftpd" = 'yes' ]; then
    cp -f $sfpanelcp/vsftpd/vsftpd.conf /etc/
...
if [ "$proftpd" = 'yes' ]; then
    echo "127.0.0.1 $servername" >> /etc/hosts
    cp -f $sfpanelcp/proftpd/proftpd.conf /etc/proftpd/
...
if [ "$mysql" = 'yes' ]; then
    ...
    cp -f $sfpanelcp/mysql/$mycnf /etc/mysql/my.cnf
    ...
    if [ "$apache" = 'yes' ]; then
        cp -f $sfpanelcp/pma/apache.conf /etc/phpmyadmin/
    ...
    if [[ ${release:0:2} -ge 18 ]]; then
        ...
        cp -f $sfpanelcp/pma/config.inc.php /etc/phpmyadmin/
    fi
...
if [ "$postgresql" = 'yes' ]; then
    ppass=$(gen_pass)
    cp -f $sfpanelcp/postgresql/pg_hba.conf /etc/postgresql/*/main/
    service postgresql restart
    sudo -u postgres psql -c "ALTER USER postgres WITH PASSWORD '$ppass'"

    # Configuring phpPgAdmin
    if [ "$apache" = 'yes' ]; then
        cp -f $sfpanelcp/pga/phppgadmin.conf /etc/apache2/conf.d/
    fi
    cp -f $sfpanelcp/pga/config.inc.php /etc/phppgadmin/
fi
...
if [ "$named" = 'yes' ]; then
    cp -f $sfpanelcp/bind/named.conf /etc/bind/
...
if [ "$exim" = 'yes' ]; then
    ...
    cp -f $sfpanelcp/exim/exim4.conf.template /etc/exim4/
    cp -f $sfpanelcp/exim/dnsbl.conf /etc/exim4/
    cp -f $sfpanelcp/exim/spam-blocks.conf /etc/exim4/
...
if [ "$dovecot" = 'yes' ]; then
    ...
    if [[ ${release:0:2} -ge 18 ]]; then
        cp -r /usr/local/sfpanel/install/debian/9/dovecot /etc/
        ...
        cp -rf $sfpanelcp/dovecot /etc/
    fi
    cp -f $sfpanelcp/logrotate/dovecot /etc/logrotate.d/
...
if [ "$clamd" = 'yes' ]; then
    ...
    cp -f $sfpanelcp/clamav/clamd.conf /etc/clamav/
    /usr/bin/freshclam
...
if [ "$exim" = 'yes' ] && [ "$mysql" = 'yes' ]; then
    if [ "$apache" = 'yes' ]; then
        cp -f $sfpanelcp/roundcube/apache.conf /etc/roundcube/
    ...
    if [[ ${release:0:2} -ge 18 ]]; then
        ...
        r="$(gen_pass)"
        cp -f $sfpanelcp/roundcube/main.inc.php /etc/roundcube/
        cp -f  $sfpanelcp/roundcube/db.inc.php /etc/roundcube/
    ...
    cp -f $sfpanelcp/roundcube/sfpanel.php \
        /usr/share/roundcube/plugins/password/drivers/
    cp -f $sfpanelcp/roundcube/config.inc.php /etc/roundcube/plugins/password/
...
# Adding Sfpanel admin account
$SFPANEL/bin/v-add-user admin $vpass $email default System Administrator
check_result $? "can't create admin user"
$SFPANEL/bin/v-change-user-shell admin bash
$SFPANEL/bin/v-change-user-language admin $lang

# Configuring system IPs
$SFPANEL/bin/v-update-sys-ip
...
if [ ! -z "$pub_ip" ] && [ "$pub_ip" != "$ip" ]; then
    echo "$SFPANEL/bin/v-update-sys-ip" >> /etc/rc.local
    $SFPANEL/bin/v-change-sys-ip-nat $ip $pub_ip
    ip=$pub_ip
fi

# Configuring MySQL/MariaDB host
if [ "$mysql" = 'yes' ]; then
    $SFPANEL/bin/v-add-database-host mysql localhost root $mpass
    $SFPANEL/bin/v-add-database admin default default $(gen_pass) mysql
fi

# Configuring PostgreSQL host
if [ "$postgresql" = 'yes' ]; then
    $SFPANEL/bin/v-add-database-host pgsql localhost postgres $ppass
    $SFPANEL/bin/v-add-database admin db db $(gen_pass) pgsql
fi

# Adding default domain
$SFPANEL/bin/v-add-domain admin $servername

# Adding cron jobs
command="sudo $SFPANEL/bin/v-update-sys-queue disk"
$SFPANEL/bin/v-add-cron-job 'admin' '15' '02' '*' '*' '*' "$command"
command="sudo $SFPANEL/bin/v-update-sys-queue traffic"
$SFPANEL/bin/v-add-cron-job 'admin' '10' '00' '*' '*' '*' "$command"
command="sudo $SFPANEL/bin/v-update-sys-queue webstats"
$SFPANEL/bin/v-add-cron-job 'admin' '30' '03' '*' '*' '*' "$command"
command="sudo $SFPANEL/bin/v-update-sys-queue backup"
$SFPANEL/bin/v-add-cron-job 'admin' '*/5' '*' '*' '*' '*' "$command"
command="sudo $SFPANEL/bin/v-backup-users"
$SFPANEL/bin/v-add-cron-job 'admin' '10' '05' '*' '*' '*' "$command"
command="sudo $SFPANEL/bin/v-update-user-stats"
$SFPANEL/bin/v-add-cron-job 'admin' '20' '00' '*' '*' '*' "$command"
command="sudo $SFPANEL/bin/v-update-sys-rrd"
$SFPANEL/bin/v-add-cron-job 'admin' '*/5' '*' '*' '*' '*' "$command"
service cron restart

# Building initital rrd images
$SFPANEL/bin/v-update-sys-rrd

# Enabling file system quota
if [ "$quota" = 'yes' ]; then
    $SFPANEL/bin/v-add-sys-quota
fi



# Starting Sfpanel service
# update-rc.d sfpanel defaults
service sfpanel start
check_result $? "sfpanel start failed"
chown admin:admin $SFPANEL/data/sessions

# Adding notifications
$SFPANEL/upd/add_notifications.sh

# Adding cronjob for autoupdates
$SFPANEL/bin/v-add-cron-sfpanel-autoupdate

if [ "$port" != "8083" ]; then
    echo "=== Set Sfpanel port: $port"
    $SFPANEL/bin/v-change-sfpanel-port $port
fi

echo "NOTIFY_ADMIN_FULL_BACKUP='$email'" >> $SFPANEL/conf/sfpanel.conf
...
if [ $make_ssl -eq 1 ]; then
    ...
    if [ "$www_host_ip" != "$pub_ip" ]; then
        if [ "$named" = 'yes' ]; then
            ...
            $SFPANEL/bin/v-delete-web-domain-alias 'admin' "$servername" "$www_host" 'no'
            $SFPANEL/bin/v-delete-dns-on-web-alias 'admin' "$servername" "$www_host" 'no'
        fi
        www_host=""
   fi
fi
...
if [ $make_ssl -eq 1 ]; then
    ...
    $SFPANEL/bin/v-add-letsencrypt-domain 'admin' "$servername" "$www_host" 'yes'
    $SFPANEL/bin/v-update-host-certificate 'admin' "$servername"
else
    echo "We will not generate SSL because of this"
fi
echo "==="
echo "UPDATE_HOSTNAME_SSL='yes'" >> $SFPANEL/conf/sfpanel.conf
fi

# Sending notification to admin email
echo -e "Congratulations, you have just successfully installed \
Sfpanel Control Panel

    https://$ip:$port
    username: admin
    password: $vpass

We hope that you enjoy your installation of Sfpanel. Please \
feel free to contact us anytime if you have any questions.
Thank you.

--
Sincerely yours
sfpanel.izetmolla.com team
" > $tmpfile

send_mail="$SFPANEL/web/inc/mail-wrapper.php"
cat $tmpfile | $send_mail -s "Sfpanel Control Panel" $email
